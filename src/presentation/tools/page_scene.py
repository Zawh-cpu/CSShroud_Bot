...
class PageScene(scene.Scene):
    ...
    async def page_previous(self, query):
        page = (await self.wizard.get_data()).get(self.__class__._page_name, 0)
    ...
    async def item_selected(self, callback_query: types.CallbackQuery, callback_data: Selector):
        await self.wizard.update_data({self.__class__._page_name: None, self.__class__._data_label: callback_query.selected})
